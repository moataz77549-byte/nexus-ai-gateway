/**
 * Analytics Service — Comprehensive analytics across all dimensions.
 *
 * Provides: usage analytics, request analytics, provider analytics, model analytics,
 * cost analytics, latency analytics, error analytics, success/failure rates,
 * user analytics, organization analytics, API analytics.
 */
...
export class AnalyticsService {
  ...
  // REQUEST ANALYTICS
  // ============================================================
  async getRequestAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
  ...
  // PROVIDER ANALYTICS
  // ============================================================
  async getProviderAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
  ...
  // MODEL ANALYTICS
  // ============================================================
  async getModelAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
    const grouped = await this.prisma.usageRecord.groupBy({
  ...
  // COST ANALYTICS
  // ============================================================
  async getCostAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
    const records = await this.prisma.usageRecord.findMany({
      where,
      orderBy: { periodStart: "asc" },
  ...
  // LATENCY ANALYTICS
  // ============================================================
  async getLatencyAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
    const records = await this.prisma.usageRecord.findMany({
      where,
      orderBy: { periodStart: "asc" },
  ...
  // ERROR ANALYTICS
  // ============================================================
  async getErrorAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
    const records = await this.prisma.usageRecord.findMany({
  ...
  // ============================================================
  async getOrganizationAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
    const grouped = await this.prisma.usageRecord.groupBy({
      by: ["organizationId"],
      where,
  ...
  // API ANALYTICS
  // ============================================================
  async getApiAnalytics(query: AnalyticsQuery) {
    const where = this.buildWhere(query);
    const grouped = await this.prisma.usageRecord.groupBy({
      by: ["apiKeyId"],
      where,
  ...
  // HELPERS
  // ============================================================
  private buildWhere(query: AnalyticsQuery): Prisma.UsageRecordWhereInput {
  ...
  private buildTimeline<T extends { periodStart: Date }>(
    records: T[],
    granularity: string,
    extractor: (r: T) => Record<string, number>
  ...
  private sumDecimal(values: Prisma.Decimal[]): Prisma.Decimal {
    return values.reduce((sum, v) => sum.add(v ?? 0), new Prisma.Decimal(0));
  }
}
